...
|  Arduino MEGA     | <-------------------->  |  Your Computer    | <----------------> |  Dashboard        |
|  (Firmware)       |    Serial @ 115200      |  (The Labrynth)   |  FastAPI/Uvicorn   |  (Browser Tab)    |
|                   |       baud              |                   |                    |                   |
+-------------------+                         +-------------------+                    +-------------------+
  Controls hardware                            Runs the FastAPI/Uvicorn                 Where you interact
  (levers, pump,                               backend; serves the React               with the experiment
   speaker, laser)                             frontend in your browser
```
...
|-----------|-----------|--------------|
| **Arduino MEGA** | A small circuit board | Controls all physical hardware (levers, pump, speaker, laser, lick circuit, imaging) |
| **Firmware** | Code uploaded to the Arduino | Listens for commands, monitors hardware, sends data back over USB |
| **The Labrynth** | Application you install on your computer | Runs the FastAPI/Uvicorn backend, serves the React frontend, and manages the connection to the Arduino |
...
The Arduino MEGA connects to each hardware component through specific pins:
...
| 7 | Cue 2 | OUTPUT | Second cue speaker output |
| 8 | Pump 2 | OUTPUT | Second infusion pump |
...
| 11 | SLM Timestamp | INPUT (PCINT) | Receives SLM timestamp signals |
| 13 | Left-Hand (LH) Lever | INPUT_PULLUP | Detects left lever presses |

> 📝 **Note:** Pin 2 is fixed (INT0) and cannot be remapped. All other pins
> are remappable.
...
- **Protocol:** Newline-delimited JSON messages sent over the USB serial connection.
  Commands from the dashboard/CLI to the firmware are numeric `CommandCode` integers
  (e.g. `CUE_ARM`=301, `PUMP_ARM`=401, `LASER_ARM`=601, `LEVER_RH_ARM`=1001,
  `LEVER_LH_ARM`=1301, `SESSION_END`=100); these are sent automatically.
- **Firmware → backend event levels:** `000` config, `001` arm/disarm, `006` error,
...
1. Download the Windows build (`dist/Labrynth/Labrynth.exe`)
2. Double-click `Labrynth.exe` to run it
3. **You may need administrator privileges** — click "Yes" if prompted by Windows
4. A desktop shortcut is created (optional)
...
1. Download the macOS build (`dist/Labrynth.app`)
2. Drag "Labrynth.app" to your Applications folder
...
4. You only need to do step 3 once
...
1. Download the Linux build (`dist/Labrynth/Labrynth`)
2. Make it executable and run it:
   ```bash
   chmod +x Labrynth
   ./Labrynth
   ```
3. If you get serial-port permission errors, see [Section 9.3](#93-linux)
...
1. Your default web browser opens automatically to `http://localhost:6229/`
2. A **system-tray icon** appears (the only native window — the dashboard itself
   runs in your browser)
3. The dashboard loads in your browser with a dark theme

> ❗ **Important:**
> Keep The Labrynth running (the system-tray icon) for the entire session.
> Quitting it from the tray shuts down the backend.
...
|---------|-------------|-----|
| System-tray icon doesn't appear | The application failed to start | Restart The Labrynth; check the terminal/console for error messages |
| Browser doesn't open automatically | No default browser set, or port 6229 already in use | Manually open your browser and go to `http://localhost:6229/`; if the port is in use, close the other application using it |
| "Server Not Running" dialog appears | The backend crashed on startup | Close and restart The Labrynth; check for other processes running on port 6229 |
...
- [ ] **Arduino MEGA** board (Mega 2560 — default; the UNO is legacy)
...
Firmware is uploaded directly from The Labrynth — you do **not** need the Arduino IDE.
The pre-compiled hex ships with the application.

### 3.2 Uploading Firmware

Firmware is uploaded from the app's **Firmware Upload** card (in the **Device Setup**
panel), which flashes pre-compiled hex to the board with `avrdude`. Follow these steps:

1. **Open the Device Setup panel** in the dashboard
2. **Select your COM port** from the **COM Port** dropdown ("Select port...")
3. **In the Firmware Upload card:**
   - Choose your **board** from the board dropdown (**MEGA** for the default Mega 2560,
     or **UNO** for legacy boards)
   - Choose your **paradigm** from the paradigm dropdown (e.g. `fr` for Fixed-Ratio)
4. **Click upload** and wait for the upload to complete

The pre-compiled hex for each board and paradigm is bundled with the application; you do
not select a port or build inside an external IDE.
...
|----------|----------------|--------|
| Fixed-Ratio (FR) | `fr/` | Current — covered in this guide |
| Progressive-Ratio (PR) | `pr/` | Current |
| Variable-Interval (VI) | `vi/` | Current |
| Omission | `omission/` | Current |
| Pavlovian | `pavlovian/` | Current |
...
| Symptom | Likely Cause | Fix |
|---------|-------------|-----|
| Board not detected | Wrong board selected or USB not connected | Select the correct board (**MEGA** or **UNO**) in the Firmware Upload card; check the USB cable is plugged in |
| Upload fails with an "avrdude" error | Wrong COM port selected or Arduino not connected | Select the correct port in the **COM Port** dropdown; try unplugging and replugging USB |
| Wrong paradigm flashed | Wrong paradigm selected in the Firmware Upload card | Re-select the correct paradigm and upload again |
...
| 4 | Pump | Use test button in Hardware Controls (Section 7.3) |
| 5 | Lick Circuit | Touch spout → check for lick events in dashboard (Section 7.5) |
| 6 | Laser | Use test button in Hardware Controls (Section 7.4) |
...
1. In the **Device Setup** panel, find the session name field
   (placeholder: "What would you like to name this session? (e.g., BOX_1)")
2. Type a **unique name** for your behavior box (e.g., "BOX_1", "Chamber_A")
3. Create the session to open it
...
2. In the **Device Setup** panel, open the **"COM Port"** dropdown ("Select port...")
3. The list of available ports appears, with the detected board
...
| Symptom | Likely Cause | Fix |
|---------|-------------|-----|
| No ports appear in dropdown | Arduino not plugged in | Plug in the USB cable, then refresh the COM Port list |
...
- The jingle only plays if the speaker is physically connected — it does not require
  arming the cue in the Hardware Controls
...
### 5.1 Hardware Controls — Arming Devices
...
| Symptom | Likely Cause | Fix |
|---------|-------------|-----|
| Device won't arm (no response when clicking) | Arduino not connected | Go to the Device Setup panel and verify connection status |
...
### 5.2 Paradigm Settings — Timing Parameters
...
### 5.3 Session Configuration — Session Limits & File Configuration
...
| SA Extinction | Time | 30 | 1 hour | 10 seconds |
| Custom | — | Set your own | Set your own | Set your own |

#### File Configuration

- **Filename:** Enter a name for your data file (e.g., `experiment1`)
- **Destination:** Choose where to save the data
- **Default save location:** your configured destination, otherwise `~/Downloads`

<details>
<summary><strong>Session Configuration Troubleshooting</strong></summary>

| Symptom | Likely Cause | Fix |
|---------|-------------|-----|
| Can't select save directory | Permissions issue | Choose a folder in your home directory (e.g., Desktop, Documents) |
| Session stops immediately after starting | Time limit set to 0 or very low value | Check the time limit in the Session Configuration |
...
| Lever presses show as TIMEOUT when they shouldn't | Timeout period is still active from the previous reward | Wait for the timeout to expire (default 20 seconds); adjust timeout in the Paradigm Settings if needed |
| All presses show as INACTIVE | Wrong lever set as active | Go to the Hardware Controls and check which lever is set as the "active" lever |

</details>

### 6.3 Pausing & Resuming

- **Pause:** Click the Pause button in the Session panel
...
1. Click the **Stop** button in the Session panel
...
|       |       |   |
|       |       |   +-- Check Session Configuration for time/infusion limits
...
2. Arm the lever (RH or LH) in the Hardware Controls
3. Press the lever
4. Check the terminal/log for press events

**Expected output format:**
```
Device: LEVER_RH | Event: PRESS | Class: ACTIVE/TIMEOUT/INACTIVE
Pin: 10 (RH) or 13 (LH)
Orientation: RH or LH
```

**In the dashboard data, events appear as:**
- `LEVER_RH` / `LEVER_LH` (device)
...
2. Arm the cue in the Hardware Controls
...
2. Arm the pump in the Hardware Controls
3. Use the test button in the Hardware Controls to run the pump
...
**Software test:**
1. Connect to the Arduino
2. Arm the laser in the Hardware Controls
...
**Software test:**
1. Connect to the Arduino
2. Arm the lick circuit in the Hardware Controls
...
**Software test:**
1. Connect to the Arduino
2. Arm the microscope in the Hardware Controls
...
4. Verify the microscope is armed in the Hardware Controls
...
3. A **ZIP archive** is saved to the destination you specified in Session Configuration
   (default: your configured destination, otherwise `~/Downloads`)

**ZIP archive contents:**

| File | What It Contains |
|------|-----------------|
| `behavior_events.csv` | Columns: device, event, start_timestamp, end_timestamp, start_frame_index, end_frame_index (segmented sessions split into `behavior_events_NNN.csv`) |
| `frame_timestamps.csv` | Columns: frame_index, timestamp_ms (if microscope was armed) |
| `slm_timestamps.csv` | Columns: event_index, timestamp_ms (if SLM was used) |
| `arduino_config.json` | Firmware info (`firmware_info`) and hardware settings (`hardware_settings`) |
| `event_log.jsonl` | Raw event log, one JSON object per line |
| `metadata.json` | Session metadata |
| `notes.txt` | Optional session notes |

### 8.2 Verifying Exported Data

After exporting, unzip the archive and open `behavior_events.csv` in any spreadsheet
program (Excel, LibreOffice Calc) or text/CSV tool:
...
2. **Verify timestamps are sequential:** Each start_timestamp should be greater than
   the previous one
3. **Cross-reference infusion count:** The number of `PUMP` / `INFUSION` events should
   match what you expected
4. **Check the metadata:** `metadata.json` records session-level details, and
   `arduino_config.json` records the firmware info and hardware settings
   (e.g., `LEVER_RH` `ACTIVE_PRESS`, `PUMP` `INFUSION`)
...
| Export fails with an error | Permission denied on the destination folder | Choose a writable directory |
| Archive extracts but `behavior_events.csv` is empty | Experiment stopped before any events occurred | Verify that events actually happened during the session |
...
| Missing behavioral events | Events may have occurred during a pause or before the program started | Check `metadata.json` and the raw `event_log.jsonl` |
| `frame_timestamps.csv` is empty | Imaging trigger not armed or microscope not sending frames | Verify microscope is armed and frame signal wiring is correct (Section 7.6) |
...
| `event_log.jsonl` | Behavioral event log, one JSON object per line |
...
- Running the application does not require admin
...
- You can add an exception in Windows Defender settings:
  ...
  the folder containing `Labrynth.exe`
...
**Port Naming:**
- Arduino MEGA typically appears as `/dev/ttyACM0`
...
|           |
|           +-- Re-flash the correct firmware from the Firmware Upload card
|               Select the fr paradigm for Fixed-Ratio experiments
|
...
|   |       |
|   |       +-- NO --> Arm the lever in the Hardware Controls
|   |       |
|   |       +-- YES --> Is the correct lever set as "active"?
|   |           |
|   |           +-- NO --> Change the active lever in the Hardware Controls
|   |           |
...
|   |       Check Hardware Controls and select the correct active lever
|   |
...
|   |       Wait for timeout to expire, or reduce it in Paradigm Settings
|   |
|   +-- No ACTIVE presses even though ratio should be met
|       |
|       +-- Check the Fixed Ratio setting in Paradigm Settings
...
+-- Timing seems wrong
    |
    +-- Check Paradigm Settings settings:
...
|   |   |
|   |   +-- YES --> Do you have write permission to the destination folder?
|   |       |
|   |       +-- Try saving to ~/Downloads or your Desktop
|   |
+-- Can't find the exported file
|   |
|   +-- Check the destination you set in Session Configuration
|   |   Default: your configured destination, otherwise ~/Downloads
|   |
|   +-- Search your computer for .zip files created today
|
...
+-- Data format questions
    ...
    |
    +-- What does the ZIP archive contain?
    |   --> See Section 8.1 for the file descriptions
    |
    +-- frame_timestamps.csv is empty?
...
| **Progressive Ratio (PR)** | A schedule where the number of presses required for each successive reward increases. (Not covered in this guide.) |
| **Variable Interval (VI)** | A schedule where rewards become available after a variable amount of time. (Not covered in this guide.) |
| **Omission** | A schedule where the animal must withhold responding to earn a reward. (Not covered in this guide.) |
